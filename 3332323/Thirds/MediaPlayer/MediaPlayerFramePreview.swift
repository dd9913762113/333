...
private final class MediaPlayerFramePreviewImpl {
    ...
    init(queue: Queue, fileReference: FileMediaReference) {
        self.queue = queue
        self.context = Promise()
        self.context.set(initializedPreviewContext(queue: queue, fileReference: fileReference))
    }
...
public final class MediaPlayerFramePreview: FramePreview {
    ...
    public init(fileReference: FileMediaReference) {
        let queue = Queue()
        self.queue = queue
        self.impl = QueueLocalObject(queue: queue, generate: {
            return MediaPlayerFramePreviewImpl(queue: queue, fileReference: fileReference)
        })
    }

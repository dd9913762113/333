...
public class MTCacheManager {
    ...
    private var cacheQueue = DispatchQueue.init(label: "com.MTCache.dev.cacheQueue")
    
    public static var sharedInstance = MTCacheManager.init(cacheName: Bundle.main.infoDictionary?["MTCache"] as? String ?? "MyAppCache")
    ...
    private init(cacheName: String) {
        if let cacheDirectory = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).last {
            let dir = cacheDirectory + "/com.MTCache/" + cacheName

...
class TheNewsViewModel {

    var controller: TheNewsViewController

    var style: Style = Settings.shared.style

    var categoryName: String = ""

    init(controller: TheNewsViewController) {
    ...
    func loadArticles(category: String = Settings.shared.category.rawValue) {
        let url = TheNewsApi.urlForCategory(category)
        TheNewsApi.getArticles(url: url) { [weak self] (articles) in
    ...
    var styleMenu: UIMenu {
        let menuActions = TheNewsViewModel.Style.allCases.map { (style) -> UIAction in
